...
aprs_data = {}
mqtt_data = {}
aprs_tx_period = datetime.timedelta(minutes=3)
mqtt_tx_period = datetime.timedelta(minutes=3)
...
interface_ip = str("192.168.152.90")    #  Sensors obtain their IP by DHCP starting from 192.168.152.100
                                        #  RaspberryPi board has fixed IP address in LAN
...
client = mqtt.Client(client_id='*******')  # TODO: Да проверя в RaspPi дали е същото!!
...
def aprs(aprs_data_string):
    ...
    prit('Sending APRS data:', msgBody)
    ptt.on()
    time.sleep(0.1)
    soc.send(msg)
    time.sleep(2)
    ...
    if (timestamp - mqtt_last_tx_timestamp) > mqtt_tx_period:
        try:
            ...
            for aprs_data_key in aprs_data:
                client.publish(aprs_data_key, aprs_data[aprs_data_key])
            client.disconnect()
